...
class TicketModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # สร้างห้อง ticket
        channel_name = f"ticket-{self.member.name}"
        ...
        await ticket_channel.send(content=f"{self.member.mention} ได้สร้างตั๋ว", embed=embed)

        # สร้างห้อง confirm สำหรับ Employee/Admin
        ...
        async def accept_callback(interaction2: discord.Interaction):
            ...
            try:
                ...
                async def finish_callback(finish_interaction: discord.Interaction):
                    ...
                    # ลบข้อความใน ticket-room ยกเว้นปุ่มเปิดตั๋ว
                    if ticket_room:
